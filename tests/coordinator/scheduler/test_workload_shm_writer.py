...
class TestCollectEntriesAndSlotMap(unittest.TestCase):
    ...
    def _make_endpoint(self, eid, tokens, kv, prefill=0):
        ep = MagicMock()
        ep.id = eid
    ...
    def test_collect_entries(self):
        ...
        ep_p = self._make_endpoint(10, 100.0, 200.0, prefill=12)
        ...
        self.assertEqual(
            entries[0],
            (1, 10, ROLE_PREFILL, 100.0, 200.0, 12),
        )
        self.assertEqual(
            entries[1],
            (2, 20, ROLE_DECODE, 300.0, 400.0, 0),
        )
        self.assertEqual(
            entries[2],
            (3, 30, ROLE_HYBRID, 500.0, 600.0, 0),
        )
...
class TestWorkloadSharedMemoryWriter(unittest.TestCase):
    ...
    def test_write_single_entry_existing_slot(self):
        ...
        self.assertEqual(entry_arg.prefill_cost, 77)
