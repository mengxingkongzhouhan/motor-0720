...
| `KvCacheAffinityPolicy` | Queries KV Conductor (via `ConductorApiClient`) for prefix match; prefers endpoints with cached blocks. ALLOCATE stamps the selected endpoint's affinity-discounted `prefill_cost` onto `Workload` (unified and load_gated; load_gated does not switch to global re-rank) | High prefix reuse, PD disaggregation |
