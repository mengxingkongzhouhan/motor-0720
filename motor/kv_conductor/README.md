...
字段的有无，`cpu_blocks` 等覆盖数值不变。有互斥 Disk 命中时下发 `disk_block_hashes`
（与 `disk_blocks` 一一对应），与该开关无关。
...
**这个拆分刻意保守**：同机不同 Pod 的块其实也便宜，但当前只按 Pod 判断，只能记作远端。
所以 `cpu_local_blocks` 是共置的**下界** —— 只会低估不会高估。高估的后果更严重：会告诉
调度器「这次搬运免费」，而实际要跨机。一机一 Pod 时，Pod 级已等价于机器级。
