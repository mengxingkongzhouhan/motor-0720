...
def test_print_wait_diagnosis_reports_missing_workload(monkeypatch, capsys):
    _fake_kubectl(monkeypatch, _kubectl_rows(("vllm-0-controller-0-675c454485-dzjqw", "Running")))

    step.print_wait_diagnosis(NAMESPACE, [], 2)
...
def test_start_monitor_diagnoses_a_stalled_wait(monkeypatch, capsys):
    ...
    monkeypatch.setattr(step, "_kubectl_output", lambda *_args: "")
