...
| `KvCacheAffinityPolicy` | Queries KV Conductor (via `ConductorApiClient`) for prefix match; prefers endpoints with cached blocks. Unified ALLOCATE stamps the selected endpoint's affinity-discounted `prefill_cost` onto `Workload` | High prefix reuse, PD disaggregation |
