...
class RequestInfo(BaseModel):
    ...
    kv_affinity_debug: dict | None = Field(
        ...
        "kv_cache_affinity policy at selection; unified forwards prefill_cost so ALLOCATE can "
        "stamp the committed endpoint ledger and re-rank by fresh load. load_gated leaves "
        "prefill_cost unset. Keyed by (instance_id, endpoint_id) tuples, so excluded from "
        "serialization.",
    )
