...
async def test_writer_snapshot_and_heartbeat(native_lib):
    ...
    try:
        ...
        class _Cache:
            def patch_workload_from_shm(self, instance_id, endpoint_id, role, active_tokens, prefill_cost=0.0, cpu_hit_blocks=0.0) -> None:
...
async def test_writer_snapshot_preserves_cas_tokens(native_lib):
    ...
    try:
        ...
        status, actual = writer.native.cas_add(
            1, 10, int(meta["generation"]), 0.0, 10.0, prefill_cost=12.0, cpu_hit_blocks=3.0
        )
        ...
        assert first["prefill_cost"] == 12.0
        assert first["cpu_hit_blocks"] == 3.0
