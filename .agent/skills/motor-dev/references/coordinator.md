...
| `KvCacheAffinityPolicy` | Queries KV Conductor (via `ConductorApiClient`) for prefix match; prefers endpoints with cached blocks. Unified ALLOCATE stamps the selected endpoint's affinity-discounted `prefill_cost` and the request ISL onto `Workload.request_tokens` (sum of in-flight request lengths; scheduler-ledger only, not in SHM). Remaining compute stays on `active_tokens` (`isl - matched`) | High prefix reuse, PD disaggregation |
...
| `SMetricGatedPolicy` | Ledger-only ranking: endpoints ordered by their ledger `workload.prefill_cost` (outstanding remaining prefill) ascending, then the first endpoint whose ledger is at or below both scaled averages over the ranked endpoints: `active_tokens <= mean * smetric_gated.active_tokens_mean_factor` and `cpu_hit_blocks <= mean * smetric_gated.cpu_hit_blocks_mean_factor` (CPU-tier KV blocks in flight; factors default 1.0; `<=` so an idle cluster still passes). Conductor is queried only for what to stamp on the committed endpoint (request `isl - matched` and `cpu_blocks`). Fallback: active gate only, then lowest ledger prefill. Always arbitrated on the Scheduler ledger (no fast path; `cpu_hit_blocks` is not in the SHM) | Prefill routing that avoids endpoints hot on remaining prefill, compute or CPU->NPU KV transfer |
